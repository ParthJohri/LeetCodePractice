...
public:
    ...
    int largestRectangleArea(vector<int>& heights,int k) {
        int n=heights.size(),ans=0,calculatedValue=0;
        vector<int> prevSmaller(n,-1),nextSmaller(n,-1);
        prevSmaller=stackPreviousSmaller(heights);
        nextSmaller=stackNextSmaller(heights);
        for(int i=0;i<n;i++){
            if(prevSmaller[i]<k and nextSmaller[i]>k)
            calculatedValue=(nextSmaller[i]-prevSmaller[i]-1)*heights[i];
            ans=max(ans,calculatedValue);
        }
        return ans;
    }
    int maximumScore(vector<int>& nums, int k) {
        int ans=largestRectangleArea(nums,k);
        return ans;
    }
};